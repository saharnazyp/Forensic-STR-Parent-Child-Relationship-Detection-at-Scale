# src/codechallenge2025/mhngar_20251220.py
"""
...
def compute_clr_bidirectional(
    ...
    for locus in locus_columns:
        ...
        lr1, status1 = compute_locus_lr(
            query_alleles, candidate_alleles, allele_freqs.get(locus, {})
        )
        log_lr_child += lr1
        ...
        lr2, status2 = compute_locus_lr(
            candidate_alleles, query_alleles, allele_freqs.get(locus, {})
        )
        log_lr_parent += lr2
    ...
    if non_missing_loci > 5:  # Only apply if we have enough data
        ...
        if both_match_ratio > 0.16:
            ...
            penalty = min(
                3.8, (both_match_ratio - 0.16) * 20.0
            )  # Lower threshold, up to 3.8 log units
...
def prefilter_candidates(
    ...
    for locus in locus_columns:
        ...
        for allele in query_alleles:
            ...
            if allele in inverted_index[locus]:
                ...
                is_rare = freq < 0.05
                weight = (
                    base_weight * 1.4 if is_rare else base_weight
                )  # 40% boost for rare alleles
    ...
    candidates_with_matches = {
        idx for idx in candidate_match_counts if candidate_match_counts[idx] > 0
    }
...
def match_single(
    ...
    if not candidate_indices:
        ...
        person_ids = database_df["PersonID"].values
        candidate_indices = [
            i for i in range(len(database_df)) if person_ids[i] != query_id
        ]
    ...
    for i, idx in enumerate(candidate_indices):
        ...
        if total_loci_checked > 0:
            ...
            if consistency_ratio > 0.59 and consistent >= 12:
                ...
                boost_factor = (
                    1.0 + (consistency_ratio - 0.59) * 2.9
                )  # Up to 1.5x boost
